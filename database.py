...
class Database():
    def __init__(self, config, time):
        self.config = config
        self.connect_string = os.getenv("DB_PATH")
            ...
            if article["postedAt"] is not None:
                ...
                if (timeToDayPost == timeToday) and check_time_in_day(timePostAtPlus) == check_time_in_day(self.time):
                    if article["type"] == "FB_POST":
                        text = article["textContent"] +  " Source_name: " + article["sourceName"] + " Facebook: " + article["link"]
                        total_paper_count += 1
                        ...
                        text = article["title"] + " Source_name: " + article["sourceName"] + " Youtube: " + article["link"] 
                        title = article["title"]
                        total_paper_count += 1
                    else:
